...
1. [Is the Original Query Accurate?](#1-is-the-original-query-accurate)
2. [Why Is It Slow?](#2-why-is-it-slow)
3. [What to Change — Optimized Query & Indexes](#3-what-to-change--optimized-query--indexes)
4. [Likely Computation Cost — Before vs After](#4-likely-computation-cost--before-vs-after)
5. [Should You Index Every Column?](#5-should-you-index-every-column)
6. [Placement Notifications in the Last 7 Days](#6-placement-notifications-in-the-last-7-days)

---

## 1. Is the Original Query Accurate?
...
**Functionally, yes — the query is logically correct.** It will return the right rows: all unread notifications belonging to student 1042, sorted newest-first. A front-end developer calling the unread-inbox endpoint will get accurate data from it.

However, correctness and efficiency are separate concerns, and this query has two practical problems beyond performance:

**Problem A — `SELECT *` pulls every column.** The application only needs to display a notification card (title, message, type, priority, createdAt, actionUrl). Selecting every column — including `metadata` (a potentially large JSONB blob), `sourceService`, `referenceId`, `referenceType`, and `expiresAt` — pushes unnecessary bytes from the database to the application on every call. At 5 million rows and dozens of concurrent users, this adds up in both network I/O and memory pressure on the DB server.

**Problem B — No `LIMIT` clause.** A student who has never marked anything as read could accumulate thousands of unread notifications over time. Without a limit, the query fetches all of them in one shot. The API contract from Stage 1 specifies pagination (`page`, `size`), but this query ignores it entirely.

So while the query returns the right answer, it is neither production-safe nor aligned with the API contract as designed.

---

## 2. Why Is It Slow?

At 5,000,000 notifications across 50,000 students that averages 100 notifications per student, but real distributions are uneven — active students may have thousands. Understanding the slowness requires looking at what the database engine actually does when this query runs without proper indexes.

### Reason 1 — Full Table Scan (the main culprit)

Without a composite index covering `(studentID, isRead)`, the database has no shortcut. It reads every single row in the `notifications` table, evaluates the `WHERE` clause on each one, and discards the ones that don't match. This is called a **sequential scan** (or full table scan).

At 5 million rows, that means:

- Reading potentially hundreds of MB of data pages from disk into the buffer pool
- Evaluating 5,000,000 comparisons to find perhaps 50–200 matching rows
- The work done is proportional to the total table size, not the result set size

This is an O(N) operation where N = 5,000,000, regardless of how few rows match.

### Reason 2 — Sort Without an Index

After filtering, the database sorts the matched rows by `createdAt DESC`. If there is no index that already has rows in that order, the engine must collect all matching rows into memory (or a temp file on disk if they don't fit) and sort them — a `filesort` in MySQL terminology, or a `Sort` node in a PostgreSQL `EXPLAIN` plan. This adds time proportional to the number of matched rows times log of that count.

### Reason 3 — Low Selectivity on `isRead`

`isRead` is a boolean — it has only two possible values. An index on `isRead` alone is nearly useless because roughly half the table matches `isRead = false` at any given time. The database query planner will often ignore such an index entirely and fall back to a sequential scan, correctly judging that it's cheaper to scan than to follow millions of index pointers.

### Reason 4 — `SELECT *` Amplifies I/O

Even once matching rows are found, fetching every column means loading full row data from heap pages. Columns like `metadata` (JSONB) can be stored out-of-line in TOAST pages in PostgreSQL, requiring additional I/O per row. A narrower `SELECT` would allow the database to satisfy the query from a covering index without touching heap pages at all.

### What the Execution Plan Looks Like (Without Index)
...
+----+-------------+---------------+------+--------+---------+-------------+
|  1 | SIMPLE      | notifications | ALL  | NULL   | 4980000 | Using where |
|    |             |               |      |        |         | Using       |
|    |             |               |      |        |         | filesort    |
+----+-------------+---------------+------+--------+---------+-------------+
...
-- rows: ~5M  → almost the entire table examined
-- Extra: "Using filesort" → in-memory or disk sort required
```

---

## 3. What to Change — Optimized Query & Indexes

### Step 1 — Create a Composite Index

The single most impactful change is adding a **composite index** that covers the exact filter and sort pattern of this query.
...
-- PostgreSQL — use a partial index (indexes only unread rows, stays small)
CREATE INDEX idx_notifications_student_unread
...
The database uses the index in three steps:
1. It seeks directly to the `studentID = 1042` section of the index — O(log N)
2. Within that section, `isRead = false` filters further (MySQL) or is already pre-filtered (PostgreSQL partial index)
3. Rows are already sorted by `createdAt DESC` inside the index, so no additional sort is needed

The engine goes from reading 5,000,000 rows to reading perhaps 50–200 index entries and fetching only those heap rows. The difference is O(N) → O(log N + K) where K is the result set size.

**Why a partial index in PostgreSQL is superior:** A standard index on `(studentID, isRead, createdAt)` indexes all 5 million rows. A partial index with `WHERE isRead = false` only indexes the unread rows — typically a much smaller set. As notifications get marked read, those rows vanish from the index automatically, keeping it compact and fast to maintain.

### Step 2 — Rewrite the Query

```sql
-- Corrected, production-safe query
SELECT
    id,
    studentID,
    title,
...
| Change | Reason |
|---|---|
| Named columns instead of `SELECT *` | Reduces I/O; enables index-only scans; makes the contract explicit |
| `LIMIT :pageSize` | Prevents unbounded result sets; aligns with the paginated API contract |
| `OFFSET :pageOffset` | Enables cursor-based pagination through the inbox |
| Named parameters (`:studentId`) | Enables query plan caching / prepared statement reuse |

### Step 3 — Companion Count Query (for pagination metadata)
...
With the composite/partial index in place, this count is served entirely from the index without touching heap pages — a very fast operation.

---

## 4. Likely Computation Cost — Before vs After

The table below compares the two execution paths at the current data volume (5,000,000 rows, 50,000 students, ~100 notifications average per student).

| Metric | Before (No Index) | After (Composite Index) |
|---|---|---|
| **Rows examined** | ~5,000,000 | ~50–200 (index entries for that student) |
| **Scan type** | Full table scan — O(N) | Index range scan — O(log N + K) |
| **Sort step** | Filesort on matched rows | Eliminated — index pre-orders rows |
| **Heap page reads** | All pages containing matched rows | Only pages for the K result rows |
| **Typical query time** | 800 ms – 3 s (cold cache) | 1 – 5 ms |
| **Scales with table growth?** | No — gets slower as rows are added | Yes — index seek cost barely changes |
| **Index maintenance cost** | None (no index exists) | Small write overhead per INSERT/UPDATE |

The write overhead of maintaining the index is negligible compared to the read gain. Every `INSERT` adds one index entry. Every `UPDATE isRead = true` removes one entry from the partial index. These are microsecond-level operations.

---

## 5. Should You Index Every Column?

**No. This advice is well-intentioned but counterproductive.** Here is a precise breakdown of why.

### What Indexes Actually Do

An index is a separate data structure (typically a B-tree) that the database maintains in parallel with the table. Every `INSERT`, `UPDATE`, and `DELETE` must update not just the table row but also every index that covers the modified columns. Indexes cost storage and write performance in exchange for read performance. That trade-off is only worthwhile when the index is actually used.

### Why Indexing Every Column Fails

**Unused indexes still cost money.** If you create 15 indexes on the `notifications` table, every single `INSERT` (a new notification arriving) must update all 15 index structures. This directly increases write latency and reduces the throughput of the bulk-create endpoint. At high write volumes this becomes a serious bottleneck.

**Low-cardinality columns make useless indexes.** Columns like `isRead` (2 values), `priority` (4 values), or `notificationType` (3 values) have very low cardinality. The database query planner will calculate that scanning the index is slower than scanning the table when a large fraction of rows match. It will simply ignore those single-column indexes at runtime.

**Index storage bloat.** Each index on a 5-million-row table consumes meaningful disk space — a B-tree index can be 30–60% the size of the table data itself. Fifteen indexes means potentially 10× the storage footprint of the data, with no proportional benefit.

**The planner can only use one index per table per query** (in most cases without index merges). Multiple single-column indexes do not combine into a compound filter the way a single composite index does.

### The Right Approach — Index Purposefully

Build indexes around access patterns, not columns:

```
Access Pattern                          → Correct Index
─────────────────────────────────────────────────────────────────────
Unread inbox for a student              → (studentID, createdAt DESC)
                                           WHERE isRead = false
All notifications of a given type      → (notificationType, createdAt DESC)
Notifications by reference entity      → (referenceId, referenceType)
                                           WHERE referenceId IS NOT NULL
TTL expiry cleanup job                 → (expiresAt)
                                           WHERE expiresAt IS NOT NULL
```

Adding an index on `title`, `message`, `sourceService`, `actionUrl`, or `metadata` individually would waste storage and hurt write performance with zero benefit, since no query filters on those columns directly.

**Summary:** Index the queries you actually run, not the columns you happen to have.

---

## 6. Placement Notifications in the Last 7 Days

The `notifications` table includes a `notificationType` column typed as a `notification_type` enum. For this system, the enum has been extended to include `'Event'`, `'Result'`, and `'Placement'` values alongside the platform types defined in Stage 1.

### Query

```sql
-- All students who received a Placement notification in the past 7 days
SELECT DISTINCT
    studentID,
    MIN(createdAt)  AS firstPlacementAlert,
    MAX(createdAt)  AS latestPlacementAlert,
    COUNT(*)        AS totalPlacementNotifications
FROM   notifications
WHERE  notificationType = 'Placement'
  AND  createdAt >= NOW() - INTERVAL 7 DAY   -- MySQL syntax
  -- PostgreSQL: createdAt >= NOW() - INTERVAL '7 days'
GROUP BY studentID
ORDER BY latestPlacementAlert DESC;
```

### What This Query Does

`WHERE notificationType = 'Placement'` narrows to placement-type rows only. `createdAt >= NOW() - INTERVAL 7 DAY` restricts to the rolling 7-day window. `GROUP BY studentID` collapses multiple notifications per student into one result row. The aggregate columns give the front-end useful context — when the student was first and last notified, and how many placement notifications they received, without needing a second query.
...
-- PostgreSQL — partial index scoped to Placement type only
CREATE INDEX idx_notifications_placement_created
    ON notifications (createdAt DESC)
    WHERE notificationType = 'Placement';
```

With this index, the database seeks directly to `notificationType = 'Placement'` entries and scans only those within the 7-day range — no full table scan needed. The result is a query that remains fast even as the table grows, because the index prunes the search space to a small, time-bounded slice of data.

### If Only a Flat Student List Is Needed

```sql
-- Simpler version: just the list of distinct student IDs
...
# Notification Fetch Optimization (Read Path Scaling)

---

## Problem Statement

Currently, notifications are fetched on every page load for every student, which leads to:
- High DB load
- Increased latency
...
Use Redis counters instead of DB COUNT queries.

**Pros:** O(1) reads  
**Cons:** Needs sync with DB  

---

### 3. Client-side Caching
Store notifications in frontend state.
...
### 5. Pagination
Fetch limited results (e.g., 20).

**Pros:** Faster queries  
**Cons:** Requires UI handling  
...
### Final Approach
Use Redis + WebSocket + Pagination + Read replicas.

---

# Stage 5

# Reliable & Scalable Notification Delivery
...
- No retries → failures lost
- Tight coupling → fragile
- No tracking → poor observability

---

## Key Design Decision

**Do NOT couple DB writes with email sending.**

---

## Improved Architecture

API → Queue → Workers (DB, Email, Push)
...
worker_db():
    ...
    while True:
        job = dequeue("notification_jobs")

        save_to_db(job.student_id, job.message)

        enqueue("delivery_jobs", job)
```
...
worker_email():
    ...
    while True:
        job = dequeue("delivery_jobs")

        try:
            send_email(job.student_id, job.message)
            log_delivery(job.notification_id, "EMAIL", "SENT")

        except:
            retry(job)
            log_delivery(job.notification_id, "EMAIL", "FAILED")
```
...
Use queue-based async processing with retries and logging.
...
## Goal
Display top N (e.g., 10) most important unread notifications based on:
...
We use a **Min Heap (size = N)**

### Why Min Heap?
- Keeps only top N elements
- Smallest element at root
- Efficient replacement when better notification arrives

---

## Algorithm

1. For each incoming notification:
   - Compute score
   - If heap size < N → push
   - Else → compare with min:
     - If higher → replace root

2. Final heap contains top N notifications
...
- Works naturally for streaming systems
- No full recomputation needed
- Always maintains real-time top N
...
| Approach | Pros | Cons |
|--------|------|------|
| Min Heap | Efficient, scalable | Slight complexity |
| Full Sort | Simple | O(N log N) expensive |
| DB Query | Easy | Not real-time |

---

## Final Recommendation

Use **Min Heap in memory** for real-time ranking  
Persist notifications in DB, but compute priority in service layer
